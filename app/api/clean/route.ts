...
function isBlockedBySSRF(parsed: URL): boolean {
  ...
  // IPv4 private ranges: 10.x, 172.16–172.31.x, 192.168.x
...
function err(code: string, error: string, extra?: Record<string, unknown>) {
  return { code, error, ...extra };
}

function logError(code: string, host: string, start: number, upstream?: number) {
  const up = upstream ?? '-';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      if (e instanceof Error && (e.name === 'AbortError' || e.cause?.name === 'AbortError')) {
